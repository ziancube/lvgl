...
static inline bool _lv_txt_cn_pretty_break(uint32_t letter_next) {
    // 。，、；：？！”）》
    static const uint32_t punctuations[] = {0x3002, 0xff0c, 0x3001, 0xff1b, 0xff1a, 0xff1f, 0xff01, 0x201d, 0xff09, 0x300b, 0};

    const uint32_t* p = punctuations;
    while (*p != 0) {
        if (letter_next == *p) {
            return false;
        }
